    ...
    for part in parts:
        ...
        if '-' in part:
            ...
            try:
                ...
                if not end_str: end_idx = total_pages - 1
                else: end_idx = int(end_str) - 1
                if not (0 <= start_idx < total_pages and 0 <= end_idx < total_pages and start_idx <= end_idx): return None 
...
def gemini_api_call_with_retry(api_function, cancel_event, status_callback, *args, **kwargs):
    retries = 0; backoff_time = INITIAL_BACKOFF
    ...
    while retries <= MAX_RETRIES_PER_CALL:
        ...
        try: return api_function(*args, **kwargs)
        except retryable_exceptions as e:
            retries += 1
            if retries > MAX_RETRIES_PER_CALL:
                status_callback(f"  Falha API após {MAX_RETRIES_PER_CALL} retentativas: {e}."); raise 
            status_callback(f"  Erro API ({type(e).__name__}): {e}. Retentativa {retries}/{MAX_RETRIES_PER_CALL} em {backoff_time}s...")
            ...
            backoff_time = min(backoff_time * 2, MAX_BACKOFF)
             ...
             status_callback(f"  Erro Recurso Esgotado API: {re_e}. Verifique cotas."); raise 
        except Exception as general_e:
            status_callback(f"  Erro inesperado chamada API: {type(general_e).__name__} - {general_e}"); raise

def pdf_to_images_local(pdf_path, output_dir, dpi, selected_pages, cancel_event, status_callback, password_request_callback, progress_callback):
    image_paths = []
    ...
    try:
        ...
        for i, page_num_0_indexed in enumerate(selected_pages):
            ...
            pix.save(output_filename); image_paths.append(output_filename)
            progress_callback(i + 1, total_selected_pages, f"Convertendo pág. PDF {page_num_0_indexed + 1}")
        status_callback("Conversão PDF->Imagens completa."); return image_paths
    except OperationCancelledError: status_callback("  Conversão PDF->Imagens cancelada."); raise 
    except Exception as e: status_callback(f"Erro conversão PDF->Imagens: {e}"); return None
    finally:
        if doc: doc.close()
    ...
    try:
        ...
        while file_to_upload.state.name == "PROCESSING":
            if cancel_event.is_set():
                ...
                except Exception as del_e: status_callback_main_thread(f"  Aviso: Falha ao deletar {file_to_upload.name}: {del_e}")
            ...
            if time.time() - processing_start_time > 300: 
                ...
                try: genai.delete_file(file_to_upload.name); status_callback_main_thread(f"  Arquivo {file_to_upload.name} com timeout deletado.")
                except Exception: pass
                return image_path, None 
            time.sleep(5)
            file_to_upload = gemini_api_call_with_retry(genai.get_file, cancel_event, status_callback_main_thread, name=file_to_upload.name)
        
        if file_to_upload.state.name == "ACTIVE": return image_path, file_to_upload
        else:
            status_callback_main_thread(f"  Falha no upload para {os.path.basename(image_path)}. Estado: {file_to_upload.state.name}")
            if file_to_upload.state.name != "DELETED":
                try: status_callback_main_thread(f"  Deletando arquivo não-ativo: {file_to_upload.name}"); genai.delete_file(file_to_upload.name)
                except Exception: pass 
            return image_path, None
    except Exception as e: 
        status_callback_main_thread(f"  Exceção na task de upload para {os.path.basename(image_path)}: {type(e).__name__} - {e}")
        return image_path, None

def upload_to_gemini_file_api(image_paths_to_upload, num_upload_workers, cancel_event, status_callback, progress_callback):
    ...
    for attempt in range(PHASE_MAX_RETRIES + 1): 
        ...
        if attempt > 0: status_callback(f"Retentativa de Upload (Fase {attempt+1}/{PHASE_MAX_RETRIES+1}) para {total_in_this_attempt} imagens...")
        else: status_callback(f"Iniciando upload concorrente de {total_in_this_attempt} imagens para '{pdf_base_name}' (workers: {num_upload_workers})...")
        ...
        for image_path in current_image_paths:
            ...
            if not mime_type:
                ...
                progress_callback(processed_for_progress_bar_this_attempt, total_in_this_attempt, f"Upload {processed_for_progress_bar_this_attempt}/{total_in_this_attempt} (Erro MIME)")
                continue
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_upload_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_path):
                ...
                try:
                    ...
                    processed_for_progress_bar_this_attempt += 1
                    progress_callback(processed_for_progress_bar_this_attempt, total_in_this_attempt, f"Upload imagem {processed_for_progress_bar_this_attempt}/{total_in_this_attempt}")
    ...
    if len(successfully_uploaded_map) < total_initial_images and total_initial_images > 0:
        status_callback(f"Aviso Final: {len(successfully_uploaded_map)}/{total_initial_images} imagens carregadas com sucesso após todas as tentativas.")
    if not successfully_uploaded_map and total_initial_images > 0:
        status_callback("Nenhuma imagem carregada com sucesso para a API após todas as tentativas."); return None
    
    status_callback(f"Upload concorrente finalizado. {len(successfully_uploaded_map)} arquivos ativos."); return successfully_uploaded_map
...
def generate_html_for_image_task(model, file_object_from_api, page_filename_local, local_img_path, cancel_event, status_callback_main_thread, current_page_num_in_doc, original_page_order_index):
    if cancel_event.is_set(): raise OperationCancelledError("Geração HTML (task) cancelada antes de iniciar.")
    try:
        img = Image.open(local_img_path)
        dimensions = img.size; img.close()
    except Exception as e: status_callback_main_thread(f"  Aviso: Falha ao ler dimensões de {page_filename_local}: {e}."); dimensions = ("unknown", "unknown")
    prompt_parts = create_html_prompt_with_desc(file_object_from_api, page_filename_local, dimensions, current_page_num_in_doc)
    generation_config = genai.types.GenerationConfig(temperature=0.1)
    try:
        response = gemini_api_call_with_retry(model.generate_content, cancel_event, status_callback_main_thread, contents=prompt_parts, generation_config=generation_config)
    except Exception as e:
        status_callback_main_thread(f"  Exceção na API Gemini para {page_filename_local} (pág {current_page_num_in_doc}): {type(e).__name__} - {e}")
    ...
    if not response.candidates:
        status_callback_main_thread(f"  Erro: Sem candidatos de resposta para {page_filename_local} (pág {current_page_num_in_doc}).")
        if hasattr(response, 'prompt_feedback'): status_callback_main_thread(f"  Feedback: {response.prompt_feedback}")
        return original_page_order_index, current_page_num_in_doc, None 
    response_text_content = response.text or ''.join(part.text for part in response.candidates[0].content.parts if hasattr(part, 'text'))
    if not response_text_content:
        status_callback_main_thread(f"  Erro: Conteúdo da resposta vazio para {page_filename_local} (pág {current_page_num_in_doc}).")
        return original_page_order_index, current_page_num_in_doc, None
    html_body = extract_html_from_response(response_text_content)
    if html_body is None:
        status_callback_main_thread(f"  Erro: Falha ao extrair HTML para {page_filename_local} (pág {current_page_num_in_doc}).")
        status_callback_main_thread(f"  Texto bruto (300c): {response_text_content[:300]}...")
        if hasattr(response.candidates[0], 'finish_reason'): status_callback_main_thread(f"  Motivo: {response.candidates[0].finish_reason}")
        return original_page_order_index, current_page_num_in_doc, None
...
def process_pdf_thread(
    ...
    image_paths = []; uploaded_files_map = {}; temp_image_dir = ""
    generated_content_list_thread = []; total_doc_pages = 0 
    num_pages_selected_by_user = 0 

    def phase_progress_callback(step, total_in_phase, text_prefix):
        overall_progress = 0; overall_total = 100 
        if "Convertendo" in text_prefix: overall_progress = int((step / total_in_phase) * 30) if total_in_phase > 0 else 0
        elif "Upload" in text_prefix: overall_progress = 30 + int((step / total_in_phase) * 30) if total_in_phase > 0 else 30
        elif "HTML" in text_prefix: overall_progress = 60 + int((step / total_in_phase) * 30) if total_in_phase > 0 else 60
        elif "Mesclando" in text_prefix or "Limpando" in text_prefix : overall_progress = 90 + int((step / total_in_phase) *10) if total_in_phase > 0 else 90
        progress_callback_gui(overall_progress, overall_total, f"{text_prefix} ({step}/{total_in_phase})")

    try:
        status_callback("Iniciando processo..."); phase_progress_callback(0, 1, "Inicializando")
        ...
        genai.configure(api_key=api_key); status_callback("Chave API Google AI configurada.")
        ...
        except Exception as e: raise Exception(f"Falha ao ler PDF: {e}")
        ...
        if num_pages_selected_by_user == 0:
            ...
            if page_range_str.strip(): raise ValueError(f"Intervalo '{page_range_str}' não resultou em páginas válidas para um PDF com {total_doc_pages} páginas.")
            raise ValueError("Nenhuma página para processar (erro interno ou PDF vazio).")

        status_callback(f"Páginas para converter (0-idx): {selected_pages_0_indexed} ({num_pages_selected_by_user} pág.)")
        
        image_paths = pdf_to_images_local(pdf_path, temp_image_dir, dpi, selected_pages_0_indexed, cancel_event, status_callback, password_request_callback_gui, phase_progress_callback)
        ...
        uploaded_files_map = upload_to_gemini_file_api(image_paths, num_upload_workers, cancel_event, status_callback, phase_progress_callback)
        ...
        sorted_uploaded_image_paths = sorted(uploaded_files_map.keys(), key=lambda x: local_path_to_original_page_num.get(x, float('inf')))
        ...
        for attempt in range(PHASE_MAX_RETRIES + 1):
            ...
            if attempt > 0: status_callback(f"Retentativa de Geração HTML (Fase {attempt+1}/{PHASE_MAX_RETRIES+1}) para {total_in_this_attempt} páginas...")
                ...
                for future in concurrent.futures.as_completed(future_to_task_info_dict):
                    ...
                    try:
                        ...
                        if returned_task_idx != idx_for_assignment:
                             status_callback(f"ALERTA CRÍTICO DE ÍNDICE: Mapeado: {idx_for_assignment}, Retornado pela Task: {returned_task_idx} para pág {page_num_doc}. USANDO ÍNDICE MAPEADO ({idx_for_assignment}).")
                        
                        if html_body:
                            if not (0 <= idx_for_assignment < len(temp_generated_html_results)): # Defensive check
                                status_callback(f"!! ERRO FATAL DE ÍNDICE !! Tentativa de usar índice {idx_for_assignment}, mas tamanho da lista é {len(temp_generated_html_results)}. Pág: {page_num_doc}. Task marcada como falha.")
                                failed_tasks_for_next_round.append(original_task_info) 
                            else:
                                temp_generated_html_results[idx_for_assignment] = {"page_num_in_doc": page_num_doc, "body": html_body}
                        else: 
                            status_callback(f"  HTML não gerado para pág. PDF {page_num_doc} (Índice Original: {idx_for_assignment}). Adicionando para retentativa de fase.")
                            failed_tasks_for_next_round.append(original_task_info)
                    except OperationCancelledError: status_callback("  Geração HTML cancelada para uma página."); failed_tasks_for_next_round.append(original_task_info); break 
                    except Exception as e: 
                        status_callback(f"  Erro ao processar resultado para pág. (índice mapeado {idx_for_assignment}), len(lista)={len(temp_generated_html_results)}: {type(e).__name__} - {e}")
                        failed_tasks_for_next_round.append(original_task_info)
                    finally:
        ...
        status_callback(f"Geração HTML finalizada. {num_successfully_generated_pages}/{num_pages_selected_by_user} págs selecionadas foram processadas.")
        ...
        if is_partial_generation:
            status_callback(f"AVISO: HTML gerado parcialmente ({num_successfully_generated_pages}/{num_pages_selected_by_user} das páginas selecionadas).")
        ...
        status_callback("Mesclando HTML..."); phase_progress_callback(0, 1, "Mesclando HTML")
        success_merge = create_merged_html_with_accessibility(generated_content_list_thread, output_html_path_final, pdf_filename_for_html_title)
    ...
    except ValueError as ve: status_callback(f"Erro Entrada/Config: {ve}"); completion_callback(False, f"Erro Entrada/Config: {ve}")
    except OperationCancelledError: status_callback("Processo cancelado."); completion_callback(False, "Processo cancelado.")
    except google_exceptions.ResourceExhausted as e: status_callback(f"Erro API (Limite Taxa): {e}."); completion_callback(False, f"Limite Taxa API: {e}.")
    except Exception as e:
        import traceback
        status_callback(f"Erro Inesperado: {e}\n{traceback.format_exc()}"); completion_callback(False, f"Erro Inesperado: {e}")
    finally:
        status_callback("Iniciando limpeza..."); phase_progress_callback(0, 1, "Limpando arquivos")
        if image_paths:
            cleaned_local = 0
        ...
        if temp_image_dir and os.path.exists(temp_image_dir):
            try: shutil.rmtree(temp_image_dir); status_callback(f"  Dir. temp. removido: {temp_image_dir}")
            except OSError as e: status_callback(f"  Aviso: Falha ao remover dir. temp. {temp_image_dir}: {e}")
        if uploaded_files_map:
            deleted_api = 0
            for file_obj in uploaded_files_map.values():
                try: genai.delete_file(file_obj.name); deleted_api+=1; time.sleep(0.2) 
                except Exception: pass
            status_callback(f"  {deleted_api} arquivos da API limpos.")
        else: status_callback("  Nenhum arquivo para limpar da API.")
        status_callback("Limpeza finalizada."); phase_progress_callback(1, 1, "Limpeza Concluída")

class OldSchoolApp:
    def __init__(self, root_window):
        self.root = root_window; self.root.title("PDF -> HTML Acessível (Gemini AI)");
        self.root.geometry("850x720"); self.root.configure(bg='black')
        self.cancel_event = threading.Event(); self.processing_thread = None
        ...
        s = ttk.Style(); s.theme_use('clam'); bg, fg, entry_bg, btn_bg, btn_active_bg = 'black', 'lime green', '#181818', '#222222', '#333333'
        ...
        s.map("TButton", background=[('active', btn_active_bg), ('pressed', btn_active_bg), ('disabled', '#444')], foreground=[('disabled', '#888')])
        s.configure("TEntry", fieldbackground=entry_bg, foreground=fg, insertcolor=fg, font=self.text_font, relief=tk.SUNKEN)
        s.configure("TFrame", background=bg)
        s.configure("Horizontal.TProgressbar", troughcolor=entry_bg, background=fg, thickness=20)
        self.root.option_add('*TCombobox*Listbox*Background', entry_bg); self.root.option_add('*TCombobox*Listbox*Foreground', fg)
        self.root.option_add('*TCombobox*Listbox*selectBackground', btn_active_bg); self.root.option_add('*TCombobox*Listbox*selectForeground', fg)
        ...
        s.map('TCombobox', fieldbackground=[('readonly', entry_bg), ('focus', entry_bg)], foreground=[('readonly', fg)],
                        selectbackground=[('readonly', entry_bg)], selectforeground=[('readonly', fg)], relief=[('readonly', tk.SUNKEN), ('focus', tk.SUNKEN)])

        mf = ttk.Frame(self.root, padding="15"); mf.pack(expand=True, fill=tk.BOTH)
        tcf = ttk.Frame(mf); tcf.pack(fill=tk.X, pady=(0,5))
        ttk.Label(tcf, text="PDF:").pack(side=tk.LEFT, padx=(0,5)); self.pdf_path_var = tk.StringVar()
        self.pdf_entry=ttk.Entry(tcf,textvariable=self.pdf_path_var,width=70); self.pdf_entry.pack(side=tk.LEFT,expand=True,fill=tk.X,padx=5)
        self.browse_btn=ttk.Button(tcf,text="Procurar...",command=self.select_pdf); self.browse_btn.pack(side=tk.LEFT,padx=(5,0))
        
        main_settings_frame = ttk.Frame(mf); main_settings_frame.pack(fill=tk.X, pady=5)
        settings_col1 = ttk.Frame(main_settings_frame); settings_col1.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,10))
        settings_col2 = ttk.Frame(main_settings_frame); settings_col2.pack(side=tk.LEFT, fill=tk.X, expand=True)

        dpi_frame = ttk.Frame(settings_col1); dpi_frame.pack(fill=tk.X, pady=2)
        ttk.Label(dpi_frame, text="DPI Imagem:").pack(side=tk.LEFT,anchor='w'); self.dpi_var=tk.StringVar(value="150")
        self.dpi_entry=ttk.Entry(dpi_frame,textvariable=self.dpi_var,width=5); self.dpi_entry.pack(side=tk.LEFT, padx=(5,0),anchor='w')

        pagerange_frame = ttk.Frame(settings_col1); pagerange_frame.pack(fill=tk.X, pady=2)
        ttk.Label(pagerange_frame,text="Págs (ex:1-3,5):").pack(side=tk.LEFT,anchor='w'); self.page_range_var=tk.StringVar()
        self.page_entry=ttk.Entry(pagerange_frame,textvariable=self.page_range_var,width=15); self.page_entry.pack(side=tk.LEFT, padx=(5,0),anchor='w')

        upload_workers_frame = ttk.Frame(settings_col1); upload_workers_frame.pack(fill=tk.X, pady=2)
        ttk.Label(upload_workers_frame, text="Upload Workers:").pack(side=tk.LEFT,anchor='w'); self.upload_workers_var = tk.StringVar(value=str(DEFAULT_UPLOAD_MAX_WORKERS))
        self.upload_workers_entry = ttk.Entry(upload_workers_frame, textvariable=self.upload_workers_var, width=4); self.upload_workers_entry.pack(side=tk.LEFT, padx=(5,0),anchor='w')

        model_frame = ttk.Frame(settings_col2); model_frame.pack(fill=tk.X, pady=2)
        ttk.Label(model_frame,text="Modelo Gemini:").pack(side=tk.LEFT,anchor='w'); self.model_var=tk.StringVar(value=DEFAULT_GEMINI_MODEL)
        self.model_cb=ttk.Combobox(model_frame,textvariable=self.model_var,values=AVAILABLE_GEMINI_MODELS,state="readonly",width=25)
        self.model_cb.pack(side=tk.LEFT, padx=(5,0),anchor='w', fill=tk.X, expand=True)

        generate_workers_frame = ttk.Frame(settings_col2); generate_workers_frame.pack(fill=tk.X, pady=2)
        ttk.Label(generate_workers_frame, text="Generate Workers:").pack(side=tk.LEFT,anchor='w'); self.generate_workers_var = tk.StringVar(value=str(DEFAULT_GENERATE_MAX_WORKERS))
        self.generate_workers_entry = ttk.Entry(generate_workers_frame, textvariable=self.generate_workers_var, width=4); self.generate_workers_entry.pack(side=tk.LEFT, padx=(5,0),anchor='w')

        abf = ttk.Frame(mf); abf.pack(fill=tk.X,pady=10)
        self.conv_btn=ttk.Button(abf,text="Converter HTML",command=self.start_conversion,style="Accent.TButton")
        s.configure("Accent.TButton",font=font.Font(family=font_family,size=11,weight="bold"),padding=8); self.conv_btn.pack(side=tk.LEFT,expand=True,fill=tk.X,padx=(0,5))
        self.cancel_btn=ttk.Button(abf,text="Cancelar",command=self.request_cancellation,state=tk.DISABLED); self.cancel_btn.pack(side=tk.LEFT,padx=5)
        self.clear_btn=ttk.Button(abf,text="Limpar Log",command=self.clear_log); self.clear_btn.pack(side=tk.LEFT,padx=(5,0))

        pf = ttk.Frame(mf); pf.pack(fill=tk.X,pady=(5,0)); self.progress_var=tk.DoubleVar(); self.progress_lbl_var=tk.StringVar(value="Pronto")
        self.prog_lbl=ttk.Label(pf,textvariable=self.progress_lbl_var,anchor=tk.W); self.prog_lbl.pack(fill=tk.X)
        self.prog_bar=ttk.Progressbar(pf,variable=self.progress_var,maximum=100,mode='determinate',style="Horizontal.TProgressbar"); self.prog_bar.pack(fill=tk.X,pady=(2,10))
        
        ttk.Label(mf,text="Log:").pack(anchor=tk.W,pady=(5,2))
        self.status_txt=scrolledtext.ScrolledText(mf,wrap=tk.WORD,height=15,bg='#0A0A0A',fg=fg,font=self.text_font,relief=tk.SUNKEN,bd=1,insertbackground=fg)
        self.status_txt.pack(expand=True,fill=tk.BOTH,pady=5); self.status_txt.configure(state='disabled')

    def select_pdf(self):
        if self.processing_thread and self.processing_thread.is_alive(): messagebox.showwarning("Em Progresso","Aguarde."); return
    ...
    def update_status(self, msg):
        def append():
            ...
            try: self.status_txt.configure(state='normal'); ts=time.strftime("%H:%M:%S"); self.status_txt.insert(tk.END,f"[{ts}] {msg}\n"); self.status_txt.configure(state='disabled'); self.status_txt.see(tk.END)
    ...
    def update_progress_bar(self, val, total, phase):
        def update():
            ...
            try: self.progress_var.set(val); self.prog_bar['maximum']=total if total > 0 else 100; self.progress_lbl_var.set(f"{phase}")
            except tk.TclError: pass
        if self.root: self.root.after(0,update)

    def request_password_from_gui(self):
        def ask(): pwd = simpledialog.askstring("PDF Protegido","Senha:",parent=self.root,show='*'); self.password_queue.put(pwd) 
        self.root.after(0,ask)
        try: return self.password_queue.get(timeout=300)
        except queue.Empty: self.update_status("Timeout ao esperar senha."); return None

    def clear_log(self):
        if self.status_txt:
            try: self.status_txt.configure(state='normal');self.status_txt.delete('1.0',tk.END);self.status_txt.configure(state='disabled');self.update_status("Log limpo.")
            except tk.TclError:pass

    def request_cancellation(self):
        if self.processing_thread and self.processing_thread.is_alive():
            if messagebox.askyesno("Confirmar","Cancelar operação?",parent=self.root):
                self.cancel_event.set();self.update_status("Cancelamento solicitado...");self.cancel_btn.config(state=tk.DISABLED,text="Cancelando...")
        else: self.update_status("Nenhuma operação para cancelar.")

    def set_controls_state(self, state):
        bs=tk.NORMAL if state=='normal' else tk.DISABLED; es='normal' if state=='normal' else 'disabled'; cs='readonly' if state=='normal' else 'disabled'
        self.conv_btn.config(state=bs);self.browse_btn.config(state=bs);self.dpi_entry.config(state=es)
        self.page_entry.config(state=es);self.model_cb.config(state=cs)
        self.upload_workers_entry.config(state=es); self.generate_workers_entry.config(state=es)
        self.cancel_btn.config(state=tk.NORMAL if state=='disabled' else tk.DISABLED,text="Cancelar" if state=='disabled' else "Cancelar")
            ...
            if success:
                ...
                try:
                    ...
                    for line in reversed(path_lines): 
                        if "HTML acessível salvo em:" in line: out_path = line.split("HTML acessível salvo em:")[-1].strip(); break
                    if not out_path and len(path_lines) > 0 : out_path = path_lines[-1].strip()
                    out_dir = os.path.dirname(out_path) if os.path.isfile(out_path) else (out_path if os.path.isdir(out_path) else "")
                    if out_dir and os.path.isdir(out_dir):
                        self.update_status(f"Abrindo dir: {out_dir}")
                        if os.name=='nt': os.startfile(out_dir)
                        elif os.name=='posix': os.system(f"{'open' if 'darwin' in os.uname().sysname.lower() else 'xdg-open'} \"{out_dir}\"")
                    else: self.update_status(f"Dir. saída não encontrado: '{out_path}'")
                except Exception as e: self.update_status(f"Falha ao abrir dir. saída: {e}")
            else: messagebox.showerror("Erro",msg,parent=self.root)
            self.update_status("--------------------"); self.update_status("Pronto.")
            self.cancel_event.clear(); self.processing_thread = None
        if self.root: self.root.after(0,final)

    def start_conversion(self):
        if self.processing_thread and self.processing_thread.is_alive(): messagebox.showwarning("Em Progresso","Conversão em andamento.");return
        pdf=self.pdf_path_var.get();dpi_s=self.dpi_var.get();pages_s=self.page_range_var.get();model_s=self.model_var.get()
        upload_w_s = self.upload_workers_var.get(); generate_w_s = self.generate_workers_var.get()

        if not pdf or not os.path.exists(pdf): messagebox.showerror("Erro","Selecione PDF válido."); return
        try: 
            dpi_i = int(dpi_s); upload_w = int(upload_w_s); generate_w = int(generate_w_s)
            if upload_w < 1 : upload_w = 1; self.upload_workers_var.set("1") 
            if generate_w < 1 : generate_w = 1; self.generate_workers_var.set("1") 
        except ValueError: messagebox.showerror("Erro","Valores de DPI/Workers devem ser numéricos."); return
        ...
        self.update_status(f"Iniciando: {pdf}"); self.update_status(f"Págs: '{pages_s if pages_s else 'Todas'}', Modelo: {model_s}, UploadW: {upload_w}, GenW: {generate_w}")
        self.set_controls_state('disabled'); self.cancel_event.clear()
        self.progress_var.set(0); self.progress_lbl_var.set("Iniciando...")
        ...
        self.processing_thread = threading.Thread(target=process_pdf_thread,
            args=(pdf,base_out,dpi_i,pages_s,model_s, upload_w, generate_w,
                  self.cancel_event,self.update_status,self.conversion_complete,self.request_password_from_gui,self.update_progress_bar))
...
if __name__ == "__main__":
    ...
    root = tk.Tk(); app = OldSchoolApp(root); root.mainloop()